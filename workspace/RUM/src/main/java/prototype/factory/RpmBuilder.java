...
public class RpmBuilder{
	...
	public void add(String name, String... functionStrings) {
		if(resources.length != functionStrings.length) {
			throw new IllegalArgumentException(String.format("RpmBuilder: nr resources wasn't equal to nr functions (%d != %d)", resources.length, functionStrings.length));
		}
		String[] prevValue = this.functions.put(name, functionStrings);
		if(prevValue != null) {
	...
	public List<RPM> buildRpms() {
		...
		for(Map.Entry<String, String[]> entry : functions.entrySet()) {
			...
			for(int i = 0; i < resources.length; i++) {			
				rpm.getResourceFunctions().put(resources[i], new ResourceFunction(entry.getValue()[i]));
			}
		}
		return result;
	}
	...
	public String toString() {
		List<String> result = functions.entrySet().stream().map(e -> String.format("    %s = {%s}", e.getKey(), String.join(", ", e.getValue()))).collect(Collectors.toList());
